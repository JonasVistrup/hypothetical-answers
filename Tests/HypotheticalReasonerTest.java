...
class HypotheticalReasonerTest {
    ...
    void evidenceTest1(){
        ...
        h.query("P(T)");
        h.nextTime("Q(0)");
        List<Answer> S0 = h.evidenceAnswers();
        h.nextTime("Q(1),R(1)");
        List<Answer> S1 = h.evidenceAnswers();
        h.nextTime("");
        List<Answer> S2 = h.evidenceAnswers();
    ...
    void evidenceTest2(){
        ...
        h.nextTime("Q(0)");
        List<Answer> S0 = h.evidenceAnswers();
        List<Answer> H = h.hypotheticalAnswers();
        h.nextTime("Q(1),R(1)");
        List<Answer> S1 = h.evidenceAnswers();
        h.nextTime("Q(2)");
        List<Answer> S2 = h.evidenceAnswers();
    ...
    void negationTest(){
        h = new HypotheticalReasoner("TempProgramNegated");
        ...
        h.addConstant("gus,john"); //TODO
        assertEquals(programString, h.toString());
        h.query("Risk(X,T)");
        assertEquals(programString+"\nHypothetical Answers:\n\t[{},{},{BCA(X,T+2),-GVS(X,T+1)}]\n" +
                "\t[{},{},{-GVS(X,T),-ST(X,us,T),-GVS(X,T+1)}]\n\n"+"Evidence Answers:\n\n"+"Answers:\n", h.toString());
        h.nextTime("GCM(gus,0)");
        assertEquals(programString+"\nHypothetical Answers:\n\t[{},{},{BCA(X,T+2),-GVS(X,T+1)}]\n" +
                "\t[{},{},{-GVS(X,T),-ST(X,us,T),-GVS(X,T+1)}]\n\n"+"Evidence Answers:\n\n"+"Answers:\n", h.toString());
    }
