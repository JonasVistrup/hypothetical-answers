...
class UpdateAnswerTest {
    ...
    void evidenceTest(){
        ...
        List<HAnswer> answerList = ModifiedSLDResolution.preprocess(p, new AtomList(query));
        AtomList T0 = new AtomList(pB.parseAtom("Q(0)"));
        AtomList T1 = new AtomList(pB.parseAtom("Q(1)"));
        T1.add(pB.parseAtom("R(1)"));

        List<EAnswer> S0 = UpdateAnswer.update(answerList, new ArrayList<>(), T0, 0);
        List<EAnswer> S1 = UpdateAnswer.update(answerList, S0, T1, 1);
        List<EAnswer> S2 = UpdateAnswer.update(answerList, S1, new AtomList(), 1);
